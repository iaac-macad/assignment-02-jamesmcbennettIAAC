...
let camera, scene, raycaster, renderer, selectedMaterialBrep, selectedMaterialMesh
...
function init() {
    ...
    // Set colors
    selectedMaterialBrep = new THREE.MeshStandardMaterial( {color: 'black'} )   
    
    selectedMaterialMesh = new THREE.MeshStandardMaterial( {color: 'darkblue'} )   
...
///////////////////////////////////////////////////////////////////////
loader.load( 'basketWeave.3dm', function ( object ) {
    ...
    document.getElementById('loader').remove()
        ...
        object.traverse( (child) => {
            if (child.userData.hasOwnProperty('objectType')) {
                if (child.userData.objectType === 'Brep, Mesh') {
                    child.traverse( (c) => {
                        if (c === child) return
...
function onClick( event ) {
    ...
    if (intersects.length > 0) {
            ...
            if (child.parent.userData.objectType === 'Brep') {
                ...
                if (child.userData.hasOwnProperty( 'material' )) {
                    ...
                    if (child.userData.objectType === 'Brep') {
                        child.material = selectedMaterialBrep
                    }
                    if (child.userData.objectType === 'Mesh') {
                        child.material = selectedMaterialMesh
                    }
                }
            }
        })
        ...
        const title = document.createElement( 'tr' )
            title.innerHTML = `<td><h3>Attributes</h3></td>`
